...
from random_trajectory_generator_3d import trajectoryCorrections
...
class ObjectTrajectory:
    ...
    def __init__(self, typObj: A ="ssphere", _STEPS: int = 10, STRIDE: float = 0.1 ):
        ...
        print("_______________init trajectory generator-------------")
        self.__trajectoryGenerator: NumpyTrajectoryGenerator = NumpyTrajectoryGenerator(_STEPS, STRIDE)
    ...
    def _object_iCubSim(self, object: iCubSim_ctrl.WorldController) -> Tuple[List, Tuple]:
        ...
        return self._object_movement(object, self.__iCubSim_wrld_ctrl, self.__trajectoryGenerator, contactSensors, self.__visualPerc, self.__sceneRecorder)
    ...
    @staticmethod
    def _object_movement(objID: iCubSim_ctrl.WorldController, iCubSim_wrld_ctrl: iCubSim_ctrl.WorldController, trajectoryGenerator: NumpyTrajectoryGenerator,
                         cont_tracker: Tuple, visual_tracker: VisualPerception, scene_recorder: SceneRecorder) -> Tuple[List, Tuple]:
        ...
        #data_list:  = trajectoryCorrections(20, 0.1)
        print(len(trajectoryGenerator))
        ...
        for i in range(len(trajectoryGenerator)):
            print("------------------------ Frame Number {}----------------------".format(i))
            for j in range(len(trajectoryGenerator[i])):
                if(j>0):
                    break
                else:
                    #-------------- move the object -----------------------------
                    iCubSim_wrld_ctrl.move_object(objID, [trajectoryGenerator[i][j], trajectoryGenerator[i][j+1], trajectoryGenerator[i][j+2]])
