...
class VisualPerception:
    def __init__(self) -> None:
        ...
        self.__leftEyeImgArray: List[np.ndarray] = [self.__left_eye_img_array]
        self.__rightEyeImgArray: List[np.ndarray] = [self.__right_eye_img_array]

    def getFinalEyeImgArrays(self) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        """ calls the function get save eye image arrays  from Outside
    ...
    def getSaveEyeImgArraysFromOutside(self, index: int = None) -> Tuple:
        """ @returns the eye image array as a numpy array from OUTSIDE THE CLASS while saving the position coordinates into the class's constructor """
        return self.__getSaveEyeimgArraysInsideTheClass(self.readCameraImagesFromOutside(index), self.__saveEyeImgArrays())

    @staticmethod
    def __getSaveEyeimgArraysInsideTheClass(readImgArray: Tuple[np.ndarray, np.ndarray], saveEyeArrays: Any) -> Tuple[np.ndarray, np.ndarray]:
